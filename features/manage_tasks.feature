...
=begin
 ...
 4 - После завершения распаковки файлов, запускаеться генерация скрин листов, статус - GENERATION.
       Уточнить может ли пользователь сам запускать эту стадию задачи и на каком этапе.
       Прдположительно пользователь может запустить повторно генерацию со статуса EXTRACTING|FINISHED
 ...
 9 - Из статуса FINISHED задача может быть пользователем переведена в состояние COMLETED
